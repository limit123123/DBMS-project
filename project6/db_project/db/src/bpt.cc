...
int db_insert(int64_t table_id, int64_t key, char* value, uint16_t val_size)
{
	...
	// can't duplicate 
	if (db_find_without_trx(table_id, key, ret_val, &size) == 0) 
		return -1;
	...
	buf_page_latch_release(table_id, 0);
	...
	{
		buf_page_latch_release(table_id, leaf_pagenum);
...
int db_find_without_trx(int64_t table_id, int64_t key, char* ret_val, uint16_t* val_size)
{
	...
	pagenum_t leaf_pagenum = db_find_leaf(table_id, key); // possible leaf_pagenum
	...
	// use delivery for offset calculation.
...
int db_find(int64_t table_id, int64_t key, char* ret_val, uint16_t* val_size, int trx_id)
{
	Slot slot;
	char delivery[PAGE_SIZE] = {};
	int result_of_locking;

	// find possible leaf page number that may have record with given key.
	pagenum_t leaf_pagenum = db_find_leaf(table_id, key); 
	
	// can't find possible leaf page
	if ( leaf_pagenum == -1 ) return -1;

	// read possible leaf page
	Leaf_page* leaf_page = (Leaf_page*)malloc(sizeof(Leaf_page));
	if ( leaf_page == NULL ) return -1;
	...
	// Since the keys are in order, use binary search to find key.
	long low = 0, high = leaf_page->numOfkeys-1, mid = 0;
	while( low <= high )
    {
        mid = (low + high) / 2;
	...
	// In case that can't find record which matched with given key.
    if ( low > high ) return -1;

	
	result_of_locking = lock_acquire(table_id, leaf_pagenum, key, trx_id, SHARED, mid, (page_t*)delivery);
	if (result_of_locking == IS_DEADLOCK) return -2; 

	if (result_of_locking == PASS_WITHOUT_PAGE) buf_read_page(table_id, leaf_pagenum, (page_t*)leaf_page);
	...
	// Since the keys are in order, use binary search to find key.
	low = 0, high = leaf_page->numOfkeys-1, mid = 0;
	while( low <= high )
    {
        mid = (low + high) / 2;
		memcpy(&slot, delivery+(SLOT_STARTING_POINT+SLOT_SIZE*mid), SLOT_SIZE);
	...
	// Read a value in the table with a matching key for transaction having trx_id.
	memcpy(ret_val, delivery+slot.offset, slot.size);
	memcpy(val_size, delivery+(SLOT_STARTING_POINT+SLOT_SIZE*mid)+sizeof(uint64_t), sizeof(uint16_t));
	...
	// operation is successfully done, and the transaction can continue the next operation.
	return 0;
}
...
int db_update(int64_t table_id, int64_t key, char* value, uint16_t new_val_size, uint16_t* old_val_size, int trx_id)
{
	...
	pagenum_t leaf_pagenum = db_find_leaf(table_id, key); 
	// if can't find possible leaf page.
	if ( leaf_pagenum == -1 ) return -1; 
	...
	result_of_locking = lock_acquire(table_id, leaf_pagenum, key, trx_id, EXCLUSIVE, mid, (page_t*)delivery);
	if (result_of_locking == IS_DEADLOCK) return -2; 

	// acquire the page latch again for doing the actual operation.
	//buf_read_page(table_id, leaf_pagenum, (page_t*)leaf_page);
	if (result_of_locking == PASS_WITHOUT_PAGE) buf_read_page(table_id, leaf_pagenum, (page_t*)leaf_page);
	...
	// If found matching ‘key’, update the value of the record to ‘values’ string with its ‘new_val_size’ 
	// and store its size in ‘old_val_size’.

	memcpy(old_val, delivery+slot.offset, slot.size);
	//trx_write_log(table_id, leaf_pagenum, key, old_val, slot.size, trx_id);
	
	memcpy(delivery+slot.offset, value, new_val_size);
	memcpy(old_val_size, delivery+(SLOT_STARTING_POINT+SLOT_SIZE*mid)+sizeof(uint64_t), sizeof(uint16_t));
	slot.size = new_val_size; 
	slot.trx_id = trx_id;
	memcpy(delivery+(SLOT_STARTING_POINT+SLOT_SIZE*mid), &slot, SLOT_SIZE);
	memcpy(delivery+24, &curr_LSN, 8);
 	memcpy(leaf_page, delivery, PAGE_SIZE);

	// Release a page latch as soon as possible, 
	buf_write_page(table_id, leaf_pagenum, (page_t*)leaf_page);

	pthread_mutex_lock(&log_manager_latch);
	make_LogRecord_2(trx_table[trx_id].last_LSN, trx_id, UPDATE, table_id, leaf_pagenum, slot.offset, slot.size, old_val, value);
	pthread_mutex_unlock(&log_manager_latch);

	trx_table[trx_id].last_LSN = curr_LSN;
...
int db_delete(int64_t table_id, int64_t key)
{
	...
	int flag = db_find_without_trx(table_id, key, ret_val, &size);
...
int init_db(int num_buf, int flag, int log_num, char* log_path, char* logmsg_path)
{
	...
	log_init_db(log_path, logmsg_path);
	buf_init_db(num_buf);

	std::list<int> losers = analysis_pass();
	redo_pass(flag, log_num);
	if (flag != 1) undo_pass(losers, flag, log_num);
...
pagenum_t db_find_leaf(int64_t table_id, int64_t key)
{
	...
	while ( internal_page->is_leaf != 1 )
	{
		...
		buf_page_latch_release(table_id, pagenum);
...
int db_insert_into_parent(int64_t table_id, pagenum_t left_pagenum, uint64_t key, pagenum_t right_pagenum, pagenum_t parent_pagenum)
{
	...
	buf_page_latch_release(table_id, parent_pagenum);
...
int db_delete_entry(int64_t table_id, pagenum_t pagenum, uint64_t key)
{
	...
	{
		//////cout << "Case : deletion from the root" << endl;
	...
	/*
	my implementation is handing locked page's number to other functions(merge or redistribution)
	And in each function, they try to buf read that page. it is a small deadlock.
	Since using mutex as is_pinned, I have to change the position of buf_page_latch_release(it was buf_pin_out).
	It is ok because in this project db_delete is not in a transaction (only single thread).
	*/

	buf_page_latch_release(table_id, del_pagenum);
	buf_page_latch_release(table_id, parent_pagenum);
	buf_page_latch_release(table_id, neighbor_pagenum);
...
int db_adjust_root(int64_t table_id, pagenum_t root_pagenum)
{
	...
	{
		buf_page_latch_release(table_id, root_pagenum);
		free(root_page);
		return 0;
	}
...
{	
	...
	if (parent_page->leftmost_pagenum == leaf_pagenum) // leaf - neighbor
	{
...
int merge_internal_pages(int64_t table_id, pagenum_t internal_pagenum, pagenum_t neighbor_pagenum, int k_prime)
{
	...
	if ( parent_page->leftmost_pagenum == internal_pagenum) // internal_page - neighbor_page.
	{
	...
	else // neighbor_page - internal_page 
	{
...
int redistribute_leaf_pages(int64_t table_id, pagenum_t leaf_pagenum, pagenum_t neighbor_pagenum, int k_prime_index)
{
	...
	if (parent_page->leftmost_pagenum != leaf_pagenum) // neighbor_page - leaf_page .
	{
...
int redistribute_internal_pages(int64_t table_id, pagenum_t internal_pagenum, pagenum_t neighbor_pagenum, int k_prime_index, int k_prime, int min_keys)
{
	...
	int i;
	if (parent_page->leftmost_pagenum != internal_pagenum ) // neighbor_page - internal_page .
	{
	...
	else // internal_page - neighbor_page
	{
