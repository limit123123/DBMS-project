...
int lock_acquire(int64_t table_id, pagenum_t page_id, int64_t key, int trx_id, int lock_mode, int record_index) 
{
	...
	{
		if (is_my_trx_already_acquired_same_or_stronger_lock_type(bucket, table_id, page_id, key, trx_id, lock_mode, record_index))
		{
			pthread_mutex_unlock(&lock_manager_latch);
			return SUCCESS;
		}
		...
		pair<int,int> result_conflict_check = is_conflict_lock_in_record_lock_list(pred_same_rid_lock_obj, table_id, page_id, trx_id, key, lock_mode, record_index); 
