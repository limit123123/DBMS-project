...
pagenum_t db_find_leaf (int64_t table_id, int64_t key)
{
	...
	file_read_page(table_id, root_pagenum, (page_t*)internal_page);
	
	pagenum_t pagenum = root_pagenum;

	int i = 0;
	while ( internal_page->is_leaf != 1 )
	{
		i = 0;
		while ( i < internal_page->numOfkeys )
		{
			if ( key >= internal_page->entries[i].key ) i++;
			else break;
		}
		if ( i == 0 ) pagenum = internal_page->leftmost_pagenum;
		else pagenum = internal_page->entries[i-1].pagenum;
		file_read_page(table_id, pagenum, (page_t*)internal_page);
	}
...
int db_start_new_tree(int64_t table_id, int64_t key, char* value, uint64_t val_size)
{
	...
	file_write_page(table_id, pagenum, (page_t*)leaf_page);
...
int db_insert_into_leaf(int64_t table_id, pagenum_t leaf_pagenum, int64_t key, char* value, uint16_t val_size)
{
	...
	insertion_point = 0;
	while( insertion_point < leaf_page->numOfkeys )
	{
		memcpy(&slot, delivery+(SLOT_STARTING_POINT+SLOT_SIZE*insertion_point), SLOT_SIZE);	
		if ( slot.key < key ) insertion_point++;
		else break;
	}
	
	// move slot one bye one 
	for ( i = leaf_page->numOfkeys; i > insertion_point; i-- )
	{
		memcpy(delivery+(SLOT_STARTING_POINT+SLOT_SIZE*i), delivery+(SLOT_STARTING_POINT+SLOT_SIZE*(i-1)), SLOT_SIZE);
	}
...
int db_insert_into_leaf_after_splitting(int64_t table_id, pagenum_t old_leaf_pagenum, int64_t key, char* value, uint16_t val_size)
{	
	Leaf_page* old_leaf_page = (Leaf_page*)malloc(sizeof(Leaf_page));
	if ( old_leaf_page == NULL ) return -1;	
	file_read_page(table_id, old_leaf_pagenum, (page_t*)old_leaf_page);
	...
	for ( j = i ; j < old_leaf_page->numOfkeys; j++ )
	{
		...
		if ( key_insert_point >= i && key < slot.key && done == 0)
		{
	...
	file_write_page(table_id, old_leaf_pagenum, (page_t*)old_leaf_page);
	file_write_page(table_id, new_leaf_pagenum, (page_t*)new_leaf_page);

	free(old_leaf_page);
	free(new_leaf_page);

	return db_insert_into_parent(table_id, old_leaf_pagenum, new_key, new_leaf_pagenum);
}


int db_insert_into_parent(int64_t table_id, pagenum_t left_pagenum, uint64_t key, pagenum_t right_pagenum)
{
	Internal_page* internal_page = (Internal_page*)malloc(sizeof(Internal_page));
	if ( internal_page == NULL ) return -1;
	file_read_page(table_id, left_pagenum, (page_t*)internal_page);

	// find parent_pagenum using internal_page(child_page)
	...
	return db_insert_into_internal_after_splitting(table_id, parent_pagenum, left_index, key, right_pagenum);
}
...
int db_insert_into_new_root(int64_t table_id, pagenum_t left_pagenum, uint64_t key, pagenum_t right_pagenum)
{
	...
	file_read_page(table_id, 0, (page_t*)header_page);
	file_read_page(table_id, left_pagenum, (page_t*)left_page);
	file_read_page(table_id, right_pagenum, (page_t*)right_page);
	...
	file_write_page(table_id, left_pagenum, (page_t*)left_page);
	file_write_page(table_id, right_pagenum, (page_t*)right_page);
...
int db_insert_into_internal_after_splitting(int64_t table_id, pagenum_t old_pagenum, int left_index, int64_t key, pagenum_t right_pagenum)
{
	Internal_page* old_page = (Internal_page*)malloc(sizeof(Internal_page));
	if ( old_page == NULL ) return -1;
	...
	file_read_page(table_id, old_pagenum, (page_t*)old_page);
	...
	new_page->parent_pagenum = old_page->parent_pagenum;
	...
	for ( i = 0; i <= new_page->numOfkeys; i++ )
	{
		if ( i == 0 )
		{
			file_read_page(table_id, new_page->leftmost_pagenum, (page_t*)child_page);
			child_page->parent_pagenum = new_pagenum;
			file_write_page(table_id, new_page->leftmost_pagenum, (page_t*)child_page);
			continue;
		}
		file_read_page(table_id, new_page->entries[i-1].pagenum, (page_t*)child_page);
		child_page->parent_pagenum = new_pagenum;
		file_write_page(table_id, new_page->entries[i-1].pagenum, (page_t*)child_page);
	}


	file_write_page(table_id, old_pagenum, (page_t*)old_page);
	...
	return db_insert_into_parent(table_id, old_pagenum, k_prime, new_pagenum);
}
...
int db_delete_entry(int64_t table_id, pagenum_t pagenum, uint64_t key)
{
	...
	if ( header_page->root_pagenum == del_pagenum ) 
	...
	file_read_page(table_id, del_pagenum, (page_t*)del_page);


	int neighbor_index = get_neighbor_index(table_id, del_pagenum);
	int k_prime_index = neighbor_index == -1 ? 0 : neighbor_index+1;
 
  	Internal_page* parent_page = (Internal_page*)malloc(sizeof(Internal_page));
  	file_read_page(table_id, del_page->parent_pagenum, (page_t*)parent_page);

	int k_prime = parent_page->entries[k_prime_index].key;


	pagenum_t neighbor_pagenum;
	if ( del_pagenum == parent_page->leftmost_pagenum )
	{
		neighbor_pagenum = parent_page->entries[0].pagenum;
	}
	else if ( del_pagenum == parent_page->entries[0].pagenum )
	{
		neighbor_pagenum = parent_page->leftmost_pagenum;
	}
	else
	{
		neighbor_pagenum = parent_page->entries[neighbor_index].pagenum;
	}
...
pagenum_t db_remove_entry_from_page(int64_t table_id, pagenum_t pagenum, uint64_t key)
{
	...
	file_read_page(table_id, pagenum, (page_t*)page);
	...
	if ( page->is_leaf == 1 )
	{
		...
		for ( i = 0; i < key_leaf_page->numOfkeys; i++ )
		{
			memcpy( &slot, delivery+(SLOT_STARTING_POINT+SLOT_SIZE*i), SLOT_SIZE );
			if ( slot.key == key ) break;
		}
		...
		memcpy( delivery+(SLOT_STARTING_POINT+SLOT_SIZE*i), cleaner, SLOT_SIZE );
		

		// moving slots one by one.
		for ( k = i+1; k < key_leaf_page->numOfkeys; k++ )
			memcpy( delivery+(SLOT_STARTING_POINT+SLOT_SIZE*(k-1)), delivery+(SLOT_STARTING_POINT+SLOT_SIZE*k), SLOT_SIZE);
		...
		file_write_page(table_id, pagenum, (page_t*)key_leaf_page);
	...
	else
	{
		...
		file_write_page(table_id, pagenum, (page_t*)internal_page);
	}
...
int db_adjust_root(int64_t table_id, pagenum_t root_pagenum)
{
	...
	if ( root_page->is_leaf == 0  )
	{
		...
		file_read_page(table_id, new_root_pagenum, (page_t*)new_root_page);
		new_root_page->parent_pagenum = 0;

		file_write_page(table_id, 0, (page_t*)header_page);
		file_write_page(table_id, new_root_pagenum, (page_t*)new_root_page);
		free(new_root_page);
	}
...
int get_neighbor_index(int64_t table_id, pagenum_t del_pagenum)
{	
	Leaf_page* del_page = (Leaf_page*)malloc(sizeof(Leaf_page));
	file_read_page(table_id, del_pagenum, (page_t*)del_page);
...
{	
	...
	Internal_page* parent_page = (Internal_page*)malloc(sizeof(Internal_page));
	if( parent_page == NULL ) return -1;
	file_read_page(table_id, leaf_page->parent_pagenum, (page_t*)parent_page);
	...
	{
		for ( i = 0; i < neighbor_page->numOfkeys; i++ )
		{
			// 이웃 페이지의 가장 왼쪽 slot
			...
			// 이웃페이지 한칸씩 앞으로 당기기. -> 나중에 한번에 옮기는걸로 수정예정.
			for( j = 1; j < neighbor_keys; j++ )
			{
				memcpy(neighbor_delivery+SLOT_STARTING_POINT+SLOT_SIZE*(j-1), neighbor_delivery+SLOT_STARTING_POINT+SLOT_SIZE*(j), SLOT_SIZE);
			}
		...
		file_write_page(table_id, leaf_pagenum, (page_t*)leaf_page);
		file_free_page(table_id, neighbor_pagenum);
	...
	else // neighbor - leaf 순서
	{
		for ( i = 0; i < leaf_page->numOfkeys; i++ )
		{
			// leaf page의 가장 왼쪽 slot
			memcpy(&slot, leaf_delivery+SLOT_STARTING_POINT, SLOT_SIZE);
			memcpy(slotted_value, leaf_delivery+slot.offset, slot.size);

			// cleaner
			memcpy(leaf_delivery+SLOT_STARTING_POINT, cleaner, SLOT_SIZE);
			memcpy(leaf_delivery+slot.offset, cleaner,slot.size);
			...
			// neighbor의 맨뒤에 slot추가.
			memcpy(neighbor_delivery+(SLOT_STARTING_POINT+SLOT_SIZE*neighbor_keys), &slot, SLOT_SIZE);
			// value add.
			memcpy(neighbor_delivery+offset, slotted_value, slot.size);
	

			// leaf page 한칸씩 앞으로 당기기.
			for( j = 1; j < leaf_keys; j++ )
			{
				memcpy(leaf_delivery+SLOT_STARTING_POINT+SLOT_SIZE*(j-1), leaf_delivery+SLOT_STARTING_POINT+SLOT_SIZE*j, SLOT_SIZE);
			}
...
int merge_internal_pages(int64_t table_id, pagenum_t internal_pagenum, pagenum_t neighbor_pagenum, int k_prime)
{
	...
	Internal_page* parent_page = (Internal_page*)malloc(sizeof(Internal_page));
    if( parent_page == NULL ) return -1;
    file_read_page(table_id, internal_page->parent_pagenum, (page_t*)parent_page);
	...
	if ( parent_page->leftmost_pagenum == internal_pagenum) // internal_page - neighbor_page 순서.
	{
		...
		file_read_page(table_id, neighbor_page->leftmost_pagenum, (page_t*)child_page);
		child_page->parent_pagenum = internal_pagenum;
		file_write_page(table_id, neighbor_page->leftmost_pagenum, (page_t*)child_page);
		...
		for ( i = insertion_index + 1, j = 0; j < end_point; i++, j++ )
		{
			...
			file_read_page(table_id, neighbor_page->entries[j].pagenum, (page_t*)child_page);
			child_page->parent_pagenum = internal_pagenum;
		   	file_write_page(table_id, neighbor_page->entries[j].pagenum, (page_t*)child_page);			
		}
		
		file_write_page(table_id, internal_pagenum, (page_t*)internal_page);
  		file_free_page(table_id, neighbor_pagenum);

		ret = db_delete_entry(table_id, internal_page->parent_pagenum, k_prime);
	}
	else // neighbor_page - internal_page 순서
	{
		...
		file_read_page(table_id, internal_page->leftmost_pagenum, (page_t*)child_page);
		child_page->parent_pagenum = neighbor_pagenum;
		file_write_page(table_id, internal_page->leftmost_pagenum, (page_t*)child_page);
		...
		for ( i = insertion_index + 1, j = 0; j < end_point; i++, j++ )
		{
			...
			file_read_page(table_id, internal_page->entries[j].pagenum, (page_t*)child_page);
			child_page->parent_pagenum = neighbor_pagenum;
			file_write_page(table_id, internal_page->entries[j].pagenum, (page_t*)child_page);
		}
...
int redistribute_leaf_pages(int64_t table_id, pagenum_t leaf_pagenum, pagenum_t neighbor_pagenum, int k_prime_index)
{
	...
	if (parent_page->leftmost_pagenum != leaf_pagenum) // neighbor_page - leaf_page 순서.
	{
		...
		{
			// 한칸씩 오른쪽으로 이동
			for ( i = leaf_keys; i > 0; i-- )
			{
				memcpy(leaf_delivery+(SLOT_STARTING_POINT+SLOT_SIZE*i), leaf_delivery+(SLOT_STARTING_POINT+SLOT_SIZE*(i-1)), SLOT_SIZE);
			}
			// 이웃 페이지의 가장 오른쪽 slot.
	...
	else // leaf_page - neighbor_page 순서
	{
		while( leaf_amount >= THRESHOLD )
		{
			...
			// 이웃페이지 한칸씩 앞으로 당기기.
			for( i = 1; i < neighbor_keys; i++ )
			{
				memcpy(neighbor_delivery+SLOT_STARTING_POINT+SLOT_SIZE*(i-1), neighbor_delivery+SLOT_STARTING_POINT+SLOT_SIZE*(i), SLOT_SIZE);
			}
    ...
    file_write_page(table_id, neighbor_pagenum, (page_t*)neighbor_page);
   	file_write_page(table_id, leaf_pagenum, (page_t*)leaf_page);
	file_write_page(table_id, leaf_page->parent_pagenum, (page_t*)parent_page);
...
int redistribute_internal_pages(int64_t table_id, pagenum_t internal_pagenum, pagenum_t neighbor_pagenum, int k_prime_index, int k_prime, int min_keys)
{
	...
	if (parent_page->leftmost_pagenum != internal_pagenum ) // neighbor_page - internal_page  순서.
	{
		...
		file_read_page( table_id, neighbor_page->entries[neighbor_page->numOfkeys-1].pagenum, (page_t*)child_page);
		child_page->parent_pagenum = internal_pagenum;
		file_write_page(table_id, neighbor_page->entries[neighbor_page->numOfkeys-1].pagenum, (page_t*)child_page); 
